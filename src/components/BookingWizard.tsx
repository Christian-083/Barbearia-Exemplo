...
import { Scissors, Calendar, Clock, User, Phone, Check, ChevronLeft, Sparkles, ShieldCheck } from 'lucide-react';
import { Service, Barber } from '../types';
import { BARBERSHOP_PHONE, BARBERS } from '../data/services';
import {
  addBooking,
  getBookings,
  findActiveSubscriptionByPhone,
...
export const BookingWizard: React.FC = () => {
  ...
  // Active Plan Auto-Detection based on phone number
  const activeSub = useMemo(() => {
    if (!customerPhone || customerPhone.length < 10) return null;
    return findActiveSubscriptionByPhone(customerPhone);
  }, [customerPhone]);
  ...
  const handleSubmitBooking = () => {
    ...
    const dateStr = formatDateBr(selectedDate);
    const paidByPlan = !!activeSub;

    if (activeSub) {
      deductSubscriptionCredit(activeSub.id);
    }
    ...
    const newBooking = {
      ...
      barberName: selectedBarber ? selectedBarber.name : 'Seu Galdino',
      paidByPlan,
      planName: activeSub ? activeSub.planName : undefined,
    };

    addBooking(newBooking);

    const planTag = paidByPlan
      ? `\n✨ *PAGO VIA PLANO:* ${activeSub?.planName} (Baixa Automática)`
      : '';
...
💰 *Valor:* ${paidByPlan ? 'INCLUSO NO PLANO RECORRENTE' : `R$ ${newBooking.price},00`}${planTag}
      ...
      <div className="bg-card border border-border rounded-3xl p-6 md:p-10 card-shadow relative overflow-hidden">
        ...
        {step === 1 && (
          <div className="flex flex-col items-center">
            <div className="w-full flex gap-4 overflow-x-auto pb-6 pt-2 px-2 no-scrollbar snap-x snap-mandatory">
              {SERVICES.map((service) => (
                <motion.div
                  ...
                  className="relative shrink-0 snap-center w-[240px] md:w-[260px] h-[320px] md:h-[360px] rounded-3xl cursor-pointer overflow-hidden group shadow-lg"
                >
                  <img
                    src={service.image}
                    alt={service.name}
                  ...
                  <div className="absolute bottom-5 left-5 right-5 flex flex-col items-start text-left z-10">
                    ...
                    <span className="font-black text-xl flex items-center gap-1">
                      <span className="text-primary">R$</span> <span className="text-primary">{service.price}</span>
                    </span>
                  </div>
                </motion.div>
              ))}
            ...
            <div className="flex items-center gap-4 w-full max-w-[280px] mt-2 opacity-50">
              <div className="h-px bg-foreground/20 flex-1"></div>
              <span className="text-xs text-foreground/50 whitespace-nowrap">← Arraste para o lado →</span>
              <div className="h-px bg-foreground/20 flex-1"></div>
            </div>
          </div>
        )}
                      ...
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-5 h-5 rounded-md border flex items-center justify-center ${
                            isSelected ? 'bg-primary border-primary text-black' : 'border-border'
                          }`}
                        >
          ...
          <div className="space-y-6 max-w-xl mx-auto">
            ...
            <div className="p-5 rounded-2xl bg-secondary/60 border border-border/80 space-y-3">
              ...
              <div className="flex justify-between items-center text-sm border-b border-border/50 pb-2">
                ...
                <span className="font-bold text-foreground">{selectedBarber ? selectedBarber.name : 'Seu Galdino'}</span>
              </div>
              <div className="flex justify-between items-center text-sm border-b border-border/50 pb-2">
              ...
              <div className="flex justify-between items-center text-base pt-1">
                ...
                {activeSub ? (
                  <span className="font-extrabold text-success text-lg flex items-center gap-1">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs uppercase tracking-widest text-muted-foreground mb-2">
                  WhatsApp (para consultar plano e enviar confirmação)
                </label>
                <div className="relative">
                ...
                {/* Auto Plan Detection Badge */}
                {activeSub && (
                  <motion.div
                    initial={{ opacity: 0, y: 5 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="mt-3 p-3 rounded-xl bg-success/15 border border-success/30 text-success text-xs flex items-center gap-2"
                  >
                    <Sparkles className="w-4 h-4 flex-shrink-0 animate-spin" />
                    <div>
                      <p className="font-bold">Plano Reconhecido: {activeSub.planName}</p>
                      <p className="opacity-90">
                        Baixa automática ativada! Este agendamento será abatido do seu plano sem cobrança.
                      </p>
                    </div>
                  </motion.div>
                )}
